...
class TestTabularLibrary:
    ...
    def test_number_of_rates(self, tl_default):
        ...
        suzuki_rates = [r for r in tl_default.get_rates() if r.rfile_name.startswith("suzuki")]
        assert len(suzuki_rates) == 61

        langanke_rates = [r for r in tl_default.get_rates() if r.rfile_name.startswith("langanke")]
        assert len(langanke_rates) == 212

        ffn_rates = [r for r in tl_default.get_rates() if r.rfile_name.startswith("ffn")]
        assert len(ffn_rates) == 88

        oda_rates = [r for r in tl_default.get_rates() if r.rfile_name.startswith("oda")]
        assert len(oda_rates) == 97

        pruet_rates = [r for r in tl_default.get_rates() if r.rfile_name.startswith("pruet")]
    ...
    def test_ordering(self):
        ...
        suzuki_rates = [r for r in tl_new.get_rates() if r.rfile_name.startswith("suzuki")]
        assert len(suzuki_rates) == 0

        langanke_rates = [r for r in tl_new.get_rates() if r.rfile_name.startswith("langanke")]
        assert len(langanke_rates) == 62

        ffn_rates = [r for r in tl_new.get_rates() if r.rfile_name.startswith("ffn")]
        assert len(ffn_rates) == 376

        oda_rates = [r for r in tl_new.get_rates() if r.rfile_name.startswith("oda")]
        assert len(oda_rates) == 20

        # these are left out
        pruet_rates = [r for r in tl_new.get_rates() if r.rfile_name.startswith("pruet")]
