    ...
    def _write_network(self, odir=None):
        ...
        with open(os.path.join(odir, "pynucastro.net"), "w") as of:
            for nuc in self.unique_nuclei:
                of.write(f"{nuc.spec_name:25} {nuc.caps_name:6} {nuc.A:6.1f} {nuc.Z:6.1f}\n")

            for nuc in self.approx_nuclei:
                of.write(f"__extra_{nuc.spec_name:17} {nuc.caps_name:6} {nuc.A:6.1f} {nuc.Z:6.1f}\n")
    ...
    def _fill_npa_index(self, n_indent, of):
        ...
        for nuc in LIG:
            if nuc in self.unique_nuclei:
                of.write(f"{self.indent*n_indent}constexpr int {nuc.caps_name}_index = {self.unique_nuclei.index(nuc)};\n")
            else:
                of.write(f"{self.indent*n_indent}constexpr int {nuc.caps_name}_index = -1;\n")
